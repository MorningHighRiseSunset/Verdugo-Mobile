  ...
  recognition.onresult = function(event) {
      ...
      for (let i = event.resultIndex; i < event.results.length; ++i) {
          ...
          if (event.results[i].isFinal) {
              ...
              spokenLetters.push(transcript);
          } else {
              interimTranscript += transcript;
          }
      }

      // Display the results with green highlight for spoken letters
      document.getElementById('result').innerHTML = finalTranscript.split('').map(letter => `<span style="background-color: lightgreen;">${letter}</span>`).join('') + '<i style="color:#999;">' + interimTranscript + '</i>';
      ...
      // Provide a suggestion based on spoken letters
      if (spokenLetters.length > 0) {
          const suggestion = getSuggestion(spokenLetters.join(''));
          document.getElementById('suggestion').innerText = suggestion ? `Did you mean: ${suggestion}?` : '';
      }
  };

  function getSuggestion(spokenLetters) {
      const possibleWords = Object.values(options).flat().map(wordObj => wordObj.word.toUpperCase());
      return possibleWords.find(word => word.startsWith(spokenLetters)) || '';
  }
